...
async function handleLogin(e) {
    ...
    try {
        ...
        if (result.role === 'manager') {
            // Jika role = manager, arahkan ke dashboard
            window.location.href = 'manager/dashboard.html'; // Redirect ke dashboard
        } else if (result.role === 'staff') {
            // Jika role = staff, tampilkan pop-up
            showPopup();
        }
    } catch (error) {
...
function showPopup() {
    ...
    popup.innerHTML = `
        <div class="popup-content">
            <p>Anda bukan manager, tolong login dengan akun manager</p>
